...
class UpdateStrapiService extends BaseService {
  constructor({ regionService, productService, redisClient, productVariantService, eventBusService }, options) {
    ...
    this.strapi_URL_STRING = `${this.protocol ?? "https"}://${this.options_.strapi_url ?? "localhost"}:${
  ...
  async addIgnore_(id, side) {
    ...
    return await this.redis_.set(key, 1, "EX", this.options_.ignore_threshold || IGNORE_THRESHOLD);
  }
